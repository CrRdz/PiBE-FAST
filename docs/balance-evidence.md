...
实现版本：`stroke-balance-evidence-v2`
...
PASS 量表及其条目不作为摄像头分数或自动标签；它们属于有人执行的时点式临床评估，
不适用于本项目的长期待机监测。
...
| 亚急性卒中两次 30 秒静站中，COP 平均速度是可靠性较好的力台指标之一 [5] | 每个有效窗口采集 30 秒；`ml_sway_mean_velocity` 为主要摆动变化特征，中央 90% 范围用于确认摆动幅度同时扩大 | 摄像头速度与力台 COP 速度数值可互换 |
...
- `ml_sway_mean_velocity`：相邻样本横向位移绝对值之和除以有效窗口时长；
- `ml_sway_p95_range`（$R_{90}$）：横向位置第 5～95 百分位范围，用于确认
  速度升高时摆动幅度也同步扩大。

窗口结果不再输出横向位置中位数、横向 RMS、单独的路径长度或躯干角 RMS；这些
描述性量没有进入当前判定，且路径长度在固定窗口内与平均速度冗余。
...
当前没有论文给出可直接迁移到 MoveNet、当前镜头和家庭人群的卒中 cutoff。因此自动
判定不采用固定肩宽比例；前五个合格窗口只用于基线校准，完成后以 baseline median
和 MAD（同时用 IQR 防止 MAD 退化）计算稳健变化分数：

```text
robust_score = |current - baseline_median| / robust_scale
```

躯干方向采用双侧变化；摆动速度和 $R_{90}$ 都只把“增加”作为变化方向。横向摆动
只有在两个变化分数同时达到 `3.5` 时才触发，以避免仅由关键点原地高频抖动造成的
速度阳性。`3.5` 是稳健异常检测中常用的 modified Z-score 界值 [8]，只具有统计
过程监测含义，不是医学 cutoff。
...
2. 用同步力台研究运动学代理与 COP 平均速度和中央摆动范围的关联；
