...
The target deployment is split by sensing requirement rather than forcing every
modality onto one low-power device and one viewpoint. B and S form the long-running
passive layer on the Raspberry Pi: B consumes an IP-camera stream reachable by the
Pi and applies throttled pose inference for low-load 24/7 observation, while S
collects rolling natural-speech windows from a microphone attached to the Pi.
Because Pi-side low-rate, distant video does not provide sufficient fine-landmark
precision, E, F, and A are short guided screens performed with a built-in or USB
webcam on a local computer. On-screen instructions elicit lateral gaze, neutral-to-
smile motion, and a bilateral arm raise under closer, more stable framing with
explicit task phases. The repository retains a single-host mode for development
and demonstration; the split configuration is the intended deployment described
here.

\begin{center}
\begin{tabular}{c}
\toprule
Pi-reachable IP camera, Pi-local microphone, and computer webcam \\
$\downarrow$ \\
Frame acquisition, resizing, orientation, and selected-input routing \\
$\downarrow$ \\
\begin{tabular}{ccc}
Pi MoveNet & Pi-local audio & Local-computer vision \\
IP camera / B & microphone / S & MediaPipe E/F; MoveNet A
\end{tabular} \\
$\downarrow$ \\
...
The backend uses Python, Flask, OpenCV, NumPy, MediaPipe, a TensorFlow Lite interpreter for MoveNet, ALSA \texttt{arecord} on Raspberry Pi, and local whisper.cpp. The Pi runs B, S, logging, and Web services; the local computer runs the guided interface and visual inference for E, F, and A. React and Vite implement the interface. Baseline and research data remain in structured files on the corresponding local host.

\subsection{Stage-based inference scheduling}

Running pose and face models continuously would waste compute and increase device temperature. In the target split deployment, the Pi keeps the B IP-camera input active and throttles MoveNet to approximately 2 frames/s by default; an independent low-load process acquires the passive S audio windows. The local computer opens its webcam only for a user-initiated screen: E/F runs Face Landmarker at approximately 5 frames/s, while A runs MoveNet. Before guided S acquires the microphone, the passive speech process pauses and then resumes afterwards. The repository's single-host demonstration mode retains stage-wise mutual exclusion of MoveNet and Face Landmarker, and preview cadence remains independent of inference cadence.
...
After a 1.5-s warm-up, the system acquires a nominal 30-s quiet-standing window. For the valid frame sequence it retains:

\begin{itemize}
  \item median trunk roll angle;
  \item mediolateral mean velocity; and
  \item the 5th--95th percentile sway range $R_{90}$.
\end{itemize}

Path length is used internally to compute mean velocity but is no longer emitted
separately; other summaries that do not enter the decision are not retained.
...
Trunk orientation is evaluated in both directions. Sway mean velocity and $R_{90}$ are abnormal only when they increase. A trunk-orientation score of 3.5 can trigger independently; mediolateral sway requires both velocity and $R_{90}$ scores to reach 3.5, reducing false alarms from high-frequency landmark jitter with little displacement. This value marks a statistical process change \cite{iglewicz1993outliers}; it is not a clinical stroke cutoff. If both MAD and IQR are zero and the later value changes, the score is undefined and the result is \texttt{insufficient}, rather than being forced by an arbitrary noise floor.

\subsection{Quality gates and decisions}

A window requires at least 30 valid samples and a valid-frame fraction of at least 0.75. Before five baseline windows have been collected, the result is \texttt{personal\_balance\_baseline\_calibrating}. Once the baseline is usable, the module reports sustained trunk-orientation change, a joint increase in mediolateral sway velocity and range, or no sustained change from the personal baseline. The direction of visible lean is reported only as a screen direction and is not translated into the side of a brain lesion.
...
Conjugate gaze deviation and quantitative eye movement abnormalities have been studied in acute stroke \cite{singer2006gaze,kudo2021eyemovement}. These studies justify evaluating the corresponding phenotypes, but they do not validate this system's numerical settings. Differences in measurement definition, acquisition device, and target population require calibration on the intended low-frame-rate webcam platform.
...
The user first looks naturally ahead. The displayed sequence then alternates centre and lateral targets so that every lateral endpoint has an immediately preceding centre endpoint. Left and right targets are each repeated three times. Three is the minimum repetition count that permits a median while retaining one atypical response. Target duration, post-transition exclusion time, and requested stimulus angle are logged acquisition variables rather than physiological or clinical limits. The interface uses physical screen width and viewing distance to place lateral targets and records the achieved, viewport-limited angle.
...
The paired lateral response is the signed difference between the lateral endpoint and its preceding centre endpoint:

\begin{equation}
r=s_{\mathrm{camera}}s_{\mathrm{direction}}
\left(g_x^{\mathrm{lateral}}-g_x^{\mathrm{preceding\ centre}}\right).
\end{equation}

Camera mirroring is fixed explicitly by the acquisition configuration rather than inferred from participant responses. For each eye and direction, the median of three responses is used. Endpoint MAD, eye-region pixel width, and between-repeat spread are reported separately as continuous quality measures; they are not combined into an unvalidated fixed SNR decision.

The implementation derives binocular directional asymmetry, direction-specific relative response difference between the two eyes, and resting gaze bias expressed as a task-calibrated proxy relative to the achieved target angle. The binocular difference is a webcam endpoint phenotype. It does not report saccade latency, peak velocity, smooth-pursuit gain, or nystagmus because a low-rate endpoint task is unsuitable for those dynamic measurements; sampling frequency strongly affects saccade analysis \cite{bahill1975mainsequence}.

\subsection{Quality measures and decision boundary}

The default build records per-trial valid/captured frames, eye-region pixel width, endpoint MAD, between-repeat spread, head-pose availability, and within-run head-pose change. Missing required landmarks or the absence of any computable endpoint produces \texttt{insufficient}; otherwise the camera record retains continuous measurements without unsupported fixed quality limits.

The camera currently applies no automatic positive/negative cutoff to resting deviation, directional-response asymmetry, or binocular endpoint difference. It reports that validation metrics were recorded. A healthy-participant technical study can estimate repeatability, failure rates, and condition-dependent distributions; diagnostic thresholds, sensitivity, and specificity require development and independent validation in acute stroke, stroke-mimic, and control cohorts. A reported sudden visual symptom still raises E directly.
...
Speakers were assigned with a fixed seed to 33 training, 7 validation, and 6 completely held-out test participants, separately by class. No speaker occurs in more than one split. Five-fold speaker-grouped predictions on the 40 development speakers provide an out-of-fold audit. The deployable coefficients are fitted only on the 33 training speakers; because deployment classifies individual recordings, the decision threshold, 0.735282, is selected from recording-level probabilities for the 7 validation speakers. The test speakers are used once for the final report. Table~\ref{tab:mdsc-results} reports both recording- and speaker-level results.
...
\midrule
Development OOF recordings ($n=16{,}200$) & 0.8241 & 0.9000 & 0.4502 & 0.6751 & 0.1942 \\
Development OOF speakers ($n=40$) & 0.9015 & 1.0000 & 0.0909 & 0.5455 & 0.1272 \\
Held-out recordings ($n=2{,}430$) & 0.9573 & 0.6724 & 0.9885 & 0.8305 & 0.0983 \\
Held-out speakers ($n=6$) & 1.0000 & 0.6667 & 1.0000 & 0.8333 & 0.0577 \\
\bottomrule
\end{tabular}
\end{table}

Recording-level uncertainty was estimated by resampling whole speakers rather than individual recordings. With only six test speakers, the 95\% intervals are 0.227--0.948 for sensitivity, 0.980--0.998 for specificity, and 0.607--0.969 for balanced accuracy; the speaker-level sensitivity interval remains 0.0--1.0 and the apparent speaker-level ROC-AUC of 1.0 is correspondingly unstable. These values are an internal research baseline, not clinical performance. On the development Mac, 100 real-WAV inference calls had a median of 0.96 ms and a 95th percentile of 1.33 ms; these are not Raspberry Pi M5 measurements.
...
Routine passive WAV files are deleted after feature extraction; the passive baseline stores numeric features rather than daily raw speech. Guided temporary audio is deleted after report handling. If configured event history preserves a positive event, it must do so under the project's explicit retention and consent policy. The 29-dimensional representation uses four metrics from a completed guided S report---CER, characters per second, pause fraction, and MDSC dysarthria probability. Passive trigger votes are not inserted into the model vector.
...
\section{Quality-Annotated Feature Fusion}
...
\begin{align}
q_E &= 0 \quad \text{(default uncalibrated mode)},\\
...
In addition to the vectors, the layer computes a threshold-normalized severity for calibrated domains. E continuous measures remain in the raw research log; until an independently validated reference exists, E is marked missing from fusion and has no domain severity. B, F, and A use their component research references:
...
PiBE-FAST implements B, E, F, A, and S as transparent temporal measurements rather than single-frame or single-score diagnosis. B compares trunk orientation and mediolateral motion with a personal robust baseline; E combines repeated paired endpoints while recording unvalidated head-motion and noise quantities; F compares smile motion against a neutral facial baseline; A requires a complete bilateral action before evaluating level and drift; and S separates sustained personal acoustic change from a locally transcribed fixed-phrase confirmation. The guided S report also carries a speaker-safe, internally evaluated MDSC chronic-dysarthria probability as a provisional S-component signal. The representation preserves measurements, quality values, and missingness as separate entries in a quality-annotated 29-dimensional vector without changing the independent urgency decision. Stage-based scheduling makes the design suitable for further Raspberry Pi evaluation, while structured result states and explicit missing masks preserve uncertainty.
...
E & acquisition timing / stimulus angle & logged acquisition variables \\
E & repetitions & three per lateral direction; median and single/triple comparison \\
E & quality / eye measures & continuous records; no default fixed decision cutoffs \\
