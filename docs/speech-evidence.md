...
固定句 S 另外加载已用官方 AISHELL-6B/MDSC 数据训练的 90 维 log-Mel
表征模型，但只输出 `shadow_dysarthria_*` 研究字段。它学习的是 MDSC 慢性构音
障碍与对照说话人的区分，不是急性卒中标签，不参与本文长期基线投票，也不改变
S 判定或紧急分级。其真实训练数据、内部持出结果和尚未完成的 M5 目标麦克风
验证见 [`mdsc-speech-training.md`](mdsc-speech-training.md)。
...
6. 最近 5 个有效窗口至少 3 票，页面才显示“建议进行固定句确认”。
