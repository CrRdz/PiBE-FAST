...
- 开发集执行说话人分组交叉验证；部署模型只拟合 train 说话人，最终阈值只使用独立
  validation 录音级预测选择。这与运行时对单条录音作判定的单位一致。
- 完全独立的测试说话人只用于最终报告。
...
|---|---:|---:|---:|---:|---:|
| 开发集折外样本（16,200 条） | 0.8241 | 0.9000 | 0.4502 | 0.6751 | 0.1942 |
| 开发集折外说话人（40 人） | 0.9015 | 1.0000 | 0.0909 | 0.5455 | 0.1272 |
| 独立测试样本（2,430 条） | 0.9573 | 0.6724 | 0.9885 | 0.8305 | 0.0983 |
| 独立测试说话人（6 人） | 1.0000 | 0.6667 | 1.0000 | 0.8333 | 0.0577 |

部署阈值只由 7 名验证说话人的 2,835 条录音校准，为 `0.735282`；验证录音级
敏感度为 0.9004、特异度为 0.8136。独立测试按说话人聚类 Bootstrap 后，录音级
敏感度 95% 区间为 `0.2272–0.9481`，特异度区间为 `0.9802–0.9975`。
说话人级独立测试仍只有 3 名阳性和 3 名阴性；
...
树莓派或目标麦克风的 M5 性能数据。仓库验证结果应以当前提交的自动化测试输出为准。

冻结测试概率的描述性校准分析得到：联合逻辑校准截距 1.101、斜率 0.802，固定斜率为
1 时的 calibration-in-the-large 为 1.465，scaled Brier score 为 0.607。这说明该内部
测试集上概率平均偏低且略显过度极端。由于测试集只有 6 名独立说话人，说话人聚类
bootstrap 中联合截距和斜率经常发生完全分离，不能给出稳定区间；这些结果不是可迁移的
临床校准。使用下列命令可从冻结模型和特征重新生成校准图、ICC 图和 JSON 摘要：

```bash
MPLCONFIGDIR=/tmp/pibefast-mpl .venv/bin/python \
  scripts/generate_paper_evidence_figures.py
```
