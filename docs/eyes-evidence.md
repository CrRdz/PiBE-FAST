...
重复误差旧版只做两次比较，在约 5 FPS 下会因一两个终点样本变化频繁拒绝健康试次。
本版改为三次取中位数并允许一个离群试次；失败报告会同时输出六组逐次响应、MAD
噪声、SNR、最近配对误差和完整离散范围，便于后续按实测分布标定。
...
本设备的 cutoff，仍属于研究参数。旧版“左眼总范围与右眼总范围相差达到某比例即
阳性”已经删除：论文中的幕上卒中方向性眼动减少主要是双眼共同的方向异常，而健康人
也可能因单目尺度、眼裂形态和关键点误差出现小幅两眼范围差。该差值仍保留在报告里，
只用于质量分析，不单独触发阳性。
